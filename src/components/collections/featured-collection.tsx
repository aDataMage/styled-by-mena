...
export function FeaturedCollection() {
  ...
  useEffect(() => {
    ...
    if (container && text && image && images) {
      ...
      tl.fromTo(container, { opacity: 0 }, { opacity: 1, duration: 1, ease: "power2.out" })
      tl.fromTo(image, { scale: 1.1, opacity: 0 }, { scale: 1, opacity: 1, duration: 1.2, ease: "power2.out" }, "-=0.8")
      tl.fromTo(
        text.children,
        { y: 30, opacity: 0 },
  ...
  return (
    <section className="mb-20" ref={containerRef}>
      <div className="relative h-[70vh] min-h-[500px] mb-12 overflow-hidden rounded-lg" ref={imageRef}>
        <Image
          src={featuredCollection.coverImage || "/placeholder.svg"}
        ...
        <div className="absolute inset-0 flex items-center justify-center p-6">
          <div className="max-w-2xl text-center text-white" ref={textRef}>
            <h2 className="font-serif text-3xl md:text-5xl mb-4">{featuredCollection.title}</h2>
            <p className="text-lg mb-6">{featuredCollection.description}</p>
            <Button asChild size="lg" className="bg-white text-black hover:bg-white/90">
              <Link href={`/collections/${featuredCollection.id}`}>Explore Collection</Link>
            </Button>
          </div>
        </div>
      </div>
